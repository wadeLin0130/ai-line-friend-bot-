...
【強制要求】
1. 請輸出更新後的 JSON，必須以「真實的 user_id」作為 key。
2. 請務必使用「繁體中文」撰寫 profile 的內容，以免後續全域關鍵字比對時因繁簡體差異而失敗！

格式範例如下：
{{
  "Ufe6857b42dce979f99f...": {{
    "name": "使用者名字",
    "profile": {{
      "喜好": "...",
      "近期狀態": "..."
...
"""
        if ENABLE_LOG:
            print("\n" + "🧠"*25, flush=True)
            print(f"[🔍 發送給 LLM 的【記憶總結】請求內容]", flush=True)
...
def process_batch(source_id, reply_token):
    ...
    # --- 結構化記憶提取 ---
    memory_prompt = ""
    # 🔥 關鍵修復：移除 enable_rag 的條件判斷，現在會無條件讀取記憶檔案
    db = load_json_db(MEMORY_FILE)
    
    if ENABLE_LOG:
        print(f"\n[🔍 記憶庫狀態] 已載入 {len(db)} 筆個人資料。", flush=True)
    ...
    if isinstance(db, dict) and db:
        ...
        for uid in current_uids:
            if uid in db:
                ...
                added_uids.add(uid)
                if ENABLE_LOG:
                    print(f"[💡 記憶命中] 提取到當前說話者 {db[uid].get('name')} 的專屬記憶！", flush=True)
            else:
                if ENABLE_LOG:
                    print(f"[❓ 記憶未命中] 當前說話者 (ID: {uid}) 不在記憶庫中。", flush=True)
        
        if config.get('features', {}).get('shared_memory', False):
            text_clean = re.sub(r'[^\w]', '', latest_msgs_text)
            keywords = set(text_clean[i:i+2] for i in range(len(text_clean)-1))
        ...
        if memory_list:
            # 💡【關鍵修正】：強烈指示 LLM 在回答問題時必須參考記憶庫
            memory_prompt = (
                "以下是你已經記住的關於使用者的重要事實與近期狀態。\n"
                + "\n".join(memory_list) + "\n\n"
            )
    ...
    reply_instruction = "1. 是否回覆(should_reply)：請務必填 true，給出回應。\n" if is_always_reply else "1. 是否回覆(should_reply)：根據你的頻率設定決定。\n"
...
def handle_incoming(event, user_text, user_id, source_id, source_type, user_name):
    is_mentioned = BOT_NAME.lower() in user_text.lower() or f"@{BOT_NAME}" in user_text
